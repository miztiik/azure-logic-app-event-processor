...
SELECT store_id, COLLECT(id) as ids
FROM [EVENT_HUB_NAME]
WHERE [discount] > 50
...
var find_suspicous_events_query =  replace(replace(find_suspicous_events_query_with_vars, '[EVENT_HUB_NAME]', '[${r_stream_analytics_job_input.name}]'), '[OutputAlias]', '[${r_output_to_svc_bus.name}]')
